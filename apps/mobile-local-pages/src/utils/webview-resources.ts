...
export function resolvePublicResourcePath(path: string) {
  const isIOS = window.navigator.userAgent.includes('iPhone') || window.navigator.userAgent.includes('iPad') || window.navigator.userAgent.includes('iPod');
  ...
  return stringUtils.unSuffix(import.meta.env.BASE_URL, '/') + stringUtils.ensurePrefix(path, '/');
}
