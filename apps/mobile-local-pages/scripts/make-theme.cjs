...
${['light', 'dark'].map(theme => {
  return Object.entries(themeColorsNext2024[theme]).map(([cssvarKey, colorValue]) => {
...
${SPACES}/* -------------------- ${theme} mode -------------------- */
${Object.entries(themeColorsNext2024[theme]).map(([cssvarKey, colorValue]) => {
  const varcore = cssvarKey.replace(/^\-\-/, '');
...
@theme {
${Object.entries(themeColorsNext2024.light).map(([cssvarKey, colorValue]) => {
    const varcore = cssvarKey.replace(/^\-\-/, '');
...
@layer theme {
  :root, :host {
    @variant dark {
${Object.entries(themeColorsNext2024.dark).map(([cssvarKey, colorValue]) => {
  const varcore = cssvarKey.replace(/^\-\-/, '');
...
`;
  fs.writeFileSync(path.resolve(ROOT, 'src/styles/tailwind-theme-v4.css'), themeContent, 'utf8');
