...
const toastBiometricsAuthenticating = ({
  isFaceIDAuth,
  isDevicePasscodeAuth,
}: {
  isFaceIDAuth: boolean;
  isDevicePasscodeAuth: boolean;
}) =>
  toastWithIcon(() => (
    <BiometricsIcon
      isFaceID={isFaceIDAuth}
      isDevicePasscode={isDevicePasscodeAuth}
      size={16}
    />
...
export function BiometricsIcon(props: {
  isFaceID?: boolean;
  isDevicePasscode?: boolean;
  size?: number;
}) {
  const {
    isFaceID = isIOS,
    isDevicePasscode = false,
    size = BiometricsIconSize,
  } = props;
  const { colors2024 } = useTheme2024();

  return isDevicePasscode ? (
    <RcIconLockCC
      width={size}
      height={size}
      ...
      try {
        ...
        if (
          ...
          hideAuthToastRef.current = toastBiometricsAuthenticating({
            ...
            isDevicePasscodeAuth: isUsingDevicePasscode,
          });
            ...
            <View style={styles.biometricsWrapper}>
              <View style={styles.biometricsBtns}>
                <TouchableView
                  ...
                  <BiometricsIcon
                    isFaceID={isFaceID}
                    isDevicePasscode={isUsingDevicePasscode}
                  />
                </TouchableView>
              </View>
            </View>
