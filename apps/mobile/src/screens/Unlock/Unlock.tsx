...
export default function UnlockScreen() {
  ...
  React.useEffect(() => {
    ...
    traceAndroidUnlockPerf('unlock_auth_state_update', {
      usingPassword,
      usingBiometrics,
      isBiometricsEnabled,
      passwordIsAutoGenerated,
      currentActionId: biometricActionIdRef.current,
    });
  }, [
    isBiometricsEnabled,
    passwordIsAutoGenerated,
  ...
  const cancelPendingBiometricUnlock = useCallback((reason: string) => {
    const previousActionId = biometricActionIdRef.current;
    biometricActionIdRef.current += 1;
    traceAndroidUnlockPerf('biometric_action_cancel', {
      reason,
      previousActionId,
      currentActionId: biometricActionIdRef.current,
      usingPassword: usingPasswordRef.current,
      isUnlocked: apisLock.isUnlocked(),
    });
  }, []);

  const isBiometricActionActive = useCallback((actionId: number) => {
    return actionId === biometricActionIdRef.current;
  }, []);
  ...
  const unlockWithBiometrics = useCallback(
    async (actionId: number) => {
      if (!isBiometricActionActive(actionId) || apisLock.isUnlocked()) {
        traceAndroidUnlockPerf('biometric_action_skip_before_request', {
          actionId,
          currentActionId: biometricActionIdRef.current,
          active: isBiometricActionActive(actionId),
          isUnlocked: apisLock.isUnlocked(),
          usingPassword: usingPasswordRef.current,
        });
        return;
      ...
      try {
        ...
        await apisKeychain.requestGenericPassword({
          ...
          onPlainPassword: async (password, credentials) => {
            if (!isBiometricActionActive(actionId)) {
              traceAndroidUnlockPerf('on_plain_password_inactive_action', {
                actionId,
                currentActionId: biometricActionIdRef.current,
                elapsedMs: Date.now() - startedAt,
                usingPassword: usingPasswordRef.current,
            ...
            try {
              ...
              if (!isBiometricActionActive(actionId)) {
                traceAndroidUnlockPerf('unlock_app_done_inactive_action', {
                  actionId,
                  currentActionId: biometricActionIdRef.current,
                  elapsedMs: Date.now() - startedAt,
                  usingPassword: usingPasswordRef.current,
        ...
        if (!isBiometricActionActive(actionId)) {
          traceAndroidUnlockPerf('request_biometrics_end_inactive_action', {
            actionId,
            currentActionId: biometricActionIdRef.current,
            elapsedMs: Date.now() - startedAt,
            usingPassword: usingPasswordRef.current,
        ...
        if (!isBiometricActionActive(actionId)) {
          traceAndroidUnlockPerf('request_biometrics_error_inactive_action', {
            actionId,
            currentActionId: biometricActionIdRef.current,
            elapsedMs: Date.now() - startedAt,
            code: error?.code,
        ...
        hidePostAuthToastRef.current?.();
        hidePostAuthToastRef.current = null;
        traceAndroidUnlockPerf('biometric_action_finish', {
          actionId,
          currentActionId: biometricActionIdRef.current,
          elapsedMs: Date.now() - startedAt,
          active: isBiometricActionActive(actionId),
  ...
  const processUnlockWithBiometrics = useCallback(async () => {
    if (
      ...
      traceAndroidUnlockPerf('biometric_action_start_skipped', {
        lockBiometric: lockBiometricRef.current,
        usingPassword: usingPasswordRef.current,
        isUnlocked: apisLock.isUnlocked(),
        currentActionId: biometricActionIdRef.current,
      });
    ...
    traceAndroidUnlockPerf('biometric_action_start', {
      actionId,
      isFaceID,
      usingPassword: usingPasswordRef.current,
      isBiometricsEnabled,
    ...
    const unlockBiometricsIfActive = () => {
      if (!isBiometricActionActive(actionId)) {
        traceAndroidUnlockPerf('biometric_action_skip_inactive', {
          actionId,
          currentActionId: biometricActionIdRef.current,
          usingPassword: usingPasswordRef.current,
          isUnlocked: apisLock.isUnlocked(),
    ...
    const releaseBiometricLock = () => {
      ...
      traceAndroidUnlockPerf('biometric_action_lock_released', {
        actionId,
        currentActionId: biometricActionIdRef.current,
        usingPassword: usingPasswordRef.current,
        isUnlocked: apisLock.isUnlocked(),
  ...
  useLayoutEffect(() => {
    ...
    const sub = perfEvents.subscribe('AUTO_TRIGGER_UNLOCK', async () => {
      ...
      if (
        ...
        traceAndroidUnlockPerf('auto_trigger_unlock_skipped', {
          pendingActionId,
          currentActionId: biometricActionIdRef.current,
          isBiometricsEnabled,
          passwordIsAutoGenerated,
            ...
            <View style={styles.formWrapper}>
              ...
              <View
                ...
                <Button
                  ...
                  onPress={evt => {
                    evt.stopPropagation();
                    cancelPendingBiometricUnlock('password_submit');
                    formik.handleSubmit();
                    checkUnlocked();
                  }}
                />
      ...
      {couldSwitchingAuthentication && (
        <View style={styles.switchingAuthTypeButtonWrapper}>
          <TouchableText
            ...
            onPress={() => {
              cancelPendingBiometricUnlock('switch_auth_type');
