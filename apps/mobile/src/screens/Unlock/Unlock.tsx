...
const AUTO_UNLOCK_FALLBACK_DELAY_MS = 2000;
...
export default function UnlockScreen({
  ...
  const {
    biometrics: { authEnabled: isBiometricsConfigured },
    computed: {
      isBiometricsEnabled,
      isFaceID,
  ...
  const couldSwitchingAuthentication =
    isBiometricsConfigured && !passwordIsAutoGenerated;
  ...
  const autoUnlockAttemptedRef = React.useRef(false);
  const autoUnlockFallbackStateRef = React.useRef({
    isBiometricsEnabled,
    isBiometricsConfigured,
    passwordIsAutoGenerated,
    couldSetupSystemAuth,
  ...
  autoUnlockFallbackStateRef.current = {
    isBiometricsEnabled,
    isBiometricsConfigured,
    passwordIsAutoGenerated,
    couldSetupSystemAuth,
  ...
  const processUnlockWithBiometrics = useCallback(async () => {
    ...
    if (
      !biometricsReady &&
      storeApisBiometrics.getBiometricsInfoSnapshot().authEnabled
    ) {
      traceAndroidUnlockPerf('biometrics_readiness_force_refresh_start', {
        elapsedMs: Date.now() - actionStartedAt,
      });
      biometricsReady = await storeApisBiometrics
        .ensureBiometricsReadyForUnlock({ forceRefreshSystemAuth: true })
        .catch(error => {
          logger.warn('[unlock] forced biometrics readiness check failed', {
            error: error instanceof Error ? error.message : String(error),
          });
          return false;
        });
      traceAndroidUnlockPerf('biometrics_readiness_force_refresh_end', {
        elapsedMs: Date.now() - actionStartedAt,
        ready: biometricsReady,
      });
    }
    if (!biometricsReady) {
      ...
      if (
        shouldShowSystemAuthUnavailableTipForUnlock({
          biometricsReady,
          isBiometricsEnabled: biometricsReady,
          passwordIsAutoGenerated,
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      return scheduleAutoUnlockFallback({
        delayMs: AUTO_UNLOCK_FALLBACK_DELAY_MS,
        shouldFallback: () => {
          const fallbackState = autoUnlockFallbackStateRef.current;
          return (
            ...
            shouldTriggerAutoUnlockFallback({
              ...
              userSelectedPasswordForSession:
                userSelectedPasswordForSessionRef.current,
            })
          );
        },
        onFallback: () => {
          logger.info('[Unlock] auto unlock fallback fired', {
            delayMs: AUTO_UNLOCK_FALLBACK_DELAY_MS,
          });
          traceAndroidUnlockPerf('auto_unlock_fallback_fired', {
            delayMs: AUTO_UNLOCK_FALLBACK_DELAY_MS,
          });
          UnlockUIManager.triggerAutoUnlock({ bypassPresentationReady: true });
        },
      });
    }, [disableAutoTriggerUnlock]),
  );
