  ...
  mode => {
    ...
    it('matches the compact Figma contract and confirms the draft value', () => {
      ...
      const inputCard = screen
        ...
        .find(node => StyleSheet.flatten(node.props.style)?.width === 202);
      expect(inputCard).toBeDefined();
      const cardStyle = StyleSheet.flatten(inputCard?.props.style);
      ...
      expect(cardStyle.backgroundColor).not.toBe(
        StyleSheet.flatten(
          screen.getByTestId('leverage-sheet').props.backgroundStyle,
        ).backgroundColor,
      );
      expect(
        StyleSheet.flatten(
          screen.getByTestId('perps-pro-leverage-increment').props.style,
        ).backgroundColor,
      ).toBe(colors['neutral-bg-5']);
      expect(
      ...
      expect(
        screen.getByTestId('perps-pro-leverage-confirm').props.height,
      ).toBe(52);
    ...
    it('keeps an empty replacement draft disabled without closing the sheet', () => {
      ...
      expect(
        screen.getByTestId('perps-pro-leverage-confirm').props
          .accessibilityState,
