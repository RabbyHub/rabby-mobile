...
import { CryptoKey, JWK } from 'react-native-quick-crypto/src/keys';
...
export async function encryptWithKey<R>(
  ...
  const vector = quickCrypto.getRandomValues(new Uint8Array(16));
  const key = unwrapKey(encryptionKey);

  const buf = await quickCrypto.subtle.encrypt(
    {
...
export async function decryptWithKey<R>(
  ...
  let decryptedObj;
  try {
    const result = await quickCrypto.subtle.decrypt(
...
export async function importKey(
  ...
  if (isExportedEncryptionKey(exportedEncryptionKey)) {
    return {
      key: await quickCrypto.subtle.importKey(
        EXPORT_FORMAT,
  ...
  return await quickCrypto.subtle.importKey(
    EXPORT_FORMAT,
...
export async function exportKey(
  ...
  if (isEncryptionKey(encryptionKey)) {
    return JSON.stringify({
      key: await quickCrypto.subtle.exportKey(EXPORT_FORMAT, encryptionKey.key),
      derivationOptions: encryptionKey.derivationOptions,
    });
  }

  return JSON.stringify(
    await quickCrypto.subtle.exportKey(EXPORT_FORMAT, encryptionKey),
  );
}
...
export async function keyFromPassword(
  ...
  try {
    ...
    const DERIVED_KEY_LENGTH = 32; // 256 bits = 32 bytes
    const DIGEST = 'sha256';

    const derivedKey = quickCrypto.pbkdf2Sync(
      passBuffer,
      saltBuffer,
      opts?.params.iterations || OLD_DERIVATION_PARAMS.params.iterations,
      DERIVED_KEY_LENGTH,
      DIGEST,
    );
...
export function generateSalt(byteCount = 32): string {
  ...
  quickCrypto.getRandomValues(view);
