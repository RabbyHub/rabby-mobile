...
import { useAssetsToken } from '@/screens/Search/useAssets';
...
import { diffLists } from '@/perfs/global-dev-helpers';
import { localTokenMetaStore } from '@/databases/hooks/token';
...
export const TokenListTurbo = ({ chain, updateToken }: Props) => {
  ...
  const { tokensByDbId, checkIsExpireAndUpdate, isLoading } = useAssetsToken();

  const statics = localTokenMetaStore(s => s.statics);
  const $fold_token_rows = localTokenMetaStore(s => s.$fold_token_rows);

  const unfold_token_rows = localTokenMetaStore(s => s.unfold_token_rows);
  const foldAndIncludeBalanceRows = localTokenMetaStore(
    s => s.fold_and_include_balance,
  );
  const foldAndExcludeBalanceRows = localTokenMetaStore(
    s => s.fold_and_exclude_balance,
  );
  const scamRows = localTokenMetaStore(s => s.scam);

  const { threshold = 0, hasExpandSwitch = false } = statics || {};
  const unFoldList = useMemo(() => {
    const list: ActionItem[] = [];
    unfold_token_rows.forEach(row => {
      if (chain && row.chain !== chain) return;
      if (!tokensByDbId[row._db_id]) return;

      list.push({
        type: 'unfold_token',
        data: combineToken(tokensByDbId[row._db_id], row.owner_addr, {
          threshold,
          hasExpandSwitch,
          unfoldTokens: statics?.unfoldTokens || [],
        }),
      });
    });
    return list;
    ...
    threshold,
    hasExpandSwitch,
    statics?.unfoldTokens,
  ]);

  const $foldTokenList = useMemo(() => {
    const list: CombineTokensItem[] = [];
    $fold_token_rows.forEach(row => {
      if (chain && row.chain !== chain) return;
      if (!tokensByDbId[row._db_id]) return;

      // list.push({ type: 'fold_token', data: tokensByDbId[row._db_id] });
      list.push(
        combineToken(tokensByDbId[row._db_id], row.owner_addr, {
          threshold,
          hasExpandSwitch,
        }),
      );
    ...
    return list;
  }, [tokensByDbId, chain, $fold_token_rows, threshold, hasExpandSwitch]);

  const foldAndIncludeBalanceList = useMemo(() => {
    const list: ActionItem[] = [];
    foldAndIncludeBalanceRows.forEach(row => {
      if (chain && row.chain !== chain) return;
      if (!tokensByDbId[row._db_id]) return;
    ...
    tokensByDbId,
    chain,
    foldAndIncludeBalanceRows,
    threshold,
    hasExpandSwitch,
  ]);

  const foldAndExcludeBalanceList = useMemo(() => {
    const list: ActionItem[] = [];
    foldAndExcludeBalanceRows.forEach(row => {
      if (chain && row.chain !== chain) return;
      if (!tokensByDbId[row._db_id]) return;
  ...
  const scamTokenList = useMemo(() => {
    ...
    return list;
  }, [tokensByDbId, chain, scamRows, threshold, hasExpandSwitch]);

  const tokenLists = {
    unFoldList,
    foldAndIncludeBalanceTokenList: foldAndIncludeBalanceList,
    foldAndExcludeBalanceTokenList: foldAndExcludeBalanceList,
    scamTokens: scamTokenList,
  };

  const tokenLen =
    unFoldList.length +
    foldAndIncludeBalanceList.length +
    foldAndExcludeBalanceList.length +
    scamTokenList.length;
  ...
  const hasNotAssets = useMemo(() => {
    return tokenLen === 0 && !isLoading && isFocused;
  }, [tokenLen, isLoading, isFocused]);
