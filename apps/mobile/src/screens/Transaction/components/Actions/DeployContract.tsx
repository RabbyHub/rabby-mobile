...
export const DeployContact: React.FC<Props> = ({ data, isSingleAddress }) => {
  ...
  const { actionData, chain } = useMemo(() => {
    const maxGasTx = data.maxGasTx;
    const actionData = maxGasTx.action!.actionData.deployContract;
    // const requireData = maxGasTx.action?.requiredData as CancelTxRequireData;
