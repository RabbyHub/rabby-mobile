# PBKDF2 600,000 次派生支持

`apps/mobile/src/core/services/encryptor.ts` 中的
`ENABLE_PBKDF2_600000` 默认是 `false`。现有 `new RNEncryptor()` 调用继续
使用 PBKDF2-SHA256、5,000 次迭代、256-bit 密钥和 AES-256-CBC。

将该常量改为 `true`，会让所有默认实例的新写入使用 600,000 次，包括本地
...
600,000 次对应 MetaMask Mobile 定义的
[OWASP2023Minimum 参数](https://github.com/MetaMask/metamask-mobile/blob/47a27b10cbabad424ea73c4936522a30d79b6b52/app/core/Encryptor/constants.ts)。
需区分支持的参数与实际启用配置：核对的 MetaMask Mobile 版本中，
[钱包初始化仍显式传入 LEGACY_DERIVATION_OPTIONS](https://github.com/MetaMask/metamask-mobile/blob/47a27b10cbabad424ea73c4936522a30d79b6b52/app/core/Engine/wallet-init/instance-options/keyring-controller.ts#L22-L24)，
即 5,000 次；600,000 并非该版本的钱包默认值。

600,000 次密文在原来的 `cipher`、`iv`、`salt` 之外增加：
...
- `decrypt` 和 `decryptWithDetail` 读取密文参数，不依赖当前开关。
  关闭开关的本版本仍可读取 600,000 次数据。
- 参数仅接受 PBKDF2 的 5,000 或 600,000 次。
  损坏、未知或过大的参数直接报错，不回退重试或调用原生派生。
- 600,000 次导出密钥也保存 `keyMetadata`；缓存解密同时检查 salt 和派生
...
分别将 5,000 次的数据升级到 600,000 次。已达到目标或更强的数据
不会被这次自动迁移降级。
...
不含本次支持的旧 App 无法通过密码解密 600,000 次密文。正式启用前需要结合
...
构造函数不执行加密，也不增加订阅或启动任务。600,000 次模式仍调用现有异步
原生 PBKDF2 接口，每次派生的迭代工作量为原来的 120 倍；首次升级还包含旧数据
...
- 移动端单测 528 个 suite / 3,899 项通过；其中 encryptor 82 项、持久化协调器 9 项。
- 前一提交的 Keyring package 单测 41 项通过（`--coverage=false`）；本次未修改该 package。
  默认命令设置的全局 100% coverage 门槛未通过，本次未降低该门槛。
...
- 前一提交的 Package ESLint 存在 131 条基线问题；当时按源代码行映射对比，未增加新问题。
