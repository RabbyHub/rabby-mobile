describe('hooks/biometrics', () => {
  ...
  it('labels Android face biometrics as face auth', async () => {
    const { module } = await setup();
    ...
    expect(computed.isFaceID).toBe(true);
    expect(computed.systemAuthTypeLabel).toBe('page.setting.faceId');
    expect(computed.systemAuthSwitchTypeLabel).toBe(
      'page.unlock.btn.switchtype_faceid',
    );
  });
});
