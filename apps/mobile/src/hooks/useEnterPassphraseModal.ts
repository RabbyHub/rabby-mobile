...
export async function promptEnterPassphrase(
  ...
  const needPassphrase = await apiMnemonic.getMnemonicKeyringIfNeedPassphrase(
    type,
    value,
  );
...
export const useEnterPassphraseModal = (type: 'address' | 'publickey') => {
  return React.useCallback(
    (value?: string) => {
      return promptEnterPassphrase(type, value);
    },
    [type],
  );
};
