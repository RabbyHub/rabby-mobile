...
import {
  ...
  setPasswordIsAutoGeneratedDurably,
} from '@/core/serviceApi/preference';
import {
  completeKeyringAuthTransition,
  getKeyringPasswordState,
...
import { planBiometricAuthStateRecovery } from '@/core/utils/biometricAuthStateRecovery';
...
export type BiometricsSetupAvailabilityConfirmation = {
...
function getBiometricsDiagnosticData() {
  const state = getBiometricsInfoSnapshot();
  return {
    authEnabled: state.authEnabled,
    authenticationType: String(getAuthenticationType()),
    devicePasscodeAvailable: state.devicePasscodeAvailable,
    passwordIsAutoGenerated: getPasswordIsAutoGeneratedSnapshot(),
    supportedBiometryType: state.supportedBiometryType ?? null,
  };
}
...
async function confirmBiometricsSetupAvailability(): Promise<BiometricsSetupAvailabilityConfirmation> {
  const cachedState = getBiometricsInfoSnapshot();
  if (cachedState.supportedBiometryType) {
    recordAuthReadinessDiagnostic('biometrics-setup-confirmed-from-cache', {
      ...getBiometricsDiagnosticData(),
    });
    return {
      couldSetupBiometrics: true,
      isBiometricsEnabled: cachedState.authEnabled,
      source: 'cache',
    };
  }

  const operationId = recordAuthReadinessDiagnostic(
  ...
  let systemAuth: BiometricsSystemAuthAvailability;
  try {
    systemAuth = await fetchSystemAuthAvailability({ forceRefresh: true });
  } catch (error) {
    recordAuthReadinessDiagnostic('biometrics-setup-refresh-failed', {
      operationId,
  ...
  const entryReady = enabledBySetting
    ? await checkAndroidBiometricsEntryReady()
    : true;
  const nextAuthEnabled = enabledBySetting && entryReady;
  ...
  const confirmation = {
    couldSetupBiometrics: !!systemAuth.supportedBiometryType,
    isBiometricsEnabled: nextAuthEnabled && !!systemAuth.supportedBiometryType,
    source: 'fresh' as const,
  };
  recordAuthReadinessDiagnostic('biometrics-setup-refresh-finished', {
    operationId,
    entryReady,
    ...confirmation,
    ...getBiometricsDiagnosticData(),
...
let authStateRecoveryPromise: Promise<void> | null = null;

export function reconcileBiometricAuthState() {
  if (authStateRecoveryPromise) {
    return authStateRecoveryPromise;
  }

  authStateRecoveryPromise = (async () => {
    const [passwordState, passwordIsAutoGenerated] = await Promise.all([
      getKeyringPasswordState(),
      getPasswordIsAutoGenerated(),
    ]);
    const plan = planBiometricAuthStateRecovery({
      passwordState,
      passwordIsAutoGenerated,
      isBiometricAuthenticationType: isAuthenticatedByBiometrics(),
    });

    if (plan.nextPasswordState) {
      await setKeyringPasswordState(plan.nextPasswordState);
    }

    if (
      plan.expectedPasswordIsAutoGenerated !== undefined &&
      plan.expectedPasswordIsAutoGenerated !== passwordIsAutoGenerated
    ) {
      await setPasswordIsAutoGeneratedDurably(
        plan.expectedPasswordIsAutoGenerated,
      );
    }

    if (plan.shouldResetKeychain) {
      await apisKeychain.resetGenericPassword();
    }

    if (
      plan.shouldCompleteDisableTransition &&
      !isAuthenticatedByBiometrics()
    ) {
      await completeKeyringAuthTransition('disable-biometrics');
    }

    if (plan.reason !== 'consistent') {
      logger.info('[biometrics] reconciled authentication state', {
        reason: plan.reason,
      });
    }
  })().finally(() => {
    authStateRecoveryPromise = null;
  });
...
async function ensureBiometricsReadyForUnlock({
  forceRefreshSystemAuth = false,
}: { forceRefreshSystemAuth?: boolean } = {}) {
  const operationId = recordAuthReadinessDiagnostic(
    'biometrics-unlock-readiness-started',
    {
      forceRefreshSystemAuth,
      ...getBiometricsDiagnosticData(),
    },
  );
  await reconcileBiometricAuthState();

  const currentState = getBiometricsInfoSnapshot();
  let systemAuth: BiometricsSystemAuthAvailability = {
    supportedBiometryType: currentState.supportedBiometryType,
    devicePasscodeAvailable: currentState.devicePasscodeAvailable,
  };
  let didFetchSystemAuth = false;
  try {
    systemAuth = await fetchSystemAuthAvailability({
      forceRefresh: forceRefreshSystemAuth,
    });
    didFetchSystemAuth = true;
  } catch (error) {
  ...
  const hasConfiguredBiometricCredential = enabledBySetting && entryReady;
  ...
  const biometricsReady =
    hasConfiguredBiometricCredential &&
    (!!systemAuth.supportedBiometryType || canUseDevicePasscode);

  setBiometrics(prev => ({
    ...prev,
    ...(didFetchSystemAuth ? systemAuth : {}),
    supportedBiometryType:
      systemAuth.supportedBiometryType ||
      (!IS_ANDROID && biometricsReady ? prev.supportedBiometryType : null),
    // Keep configured credential state separate from the system's current
    // availability. Some Android devices can report a transient negative
    // during cold start; that result must not erase unlock retry eligibility.
    authEnabled: hasConfiguredBiometricCredential,
  }));

  recordAuthReadinessDiagnostic('biometrics-unlock-readiness-finished', {
    operationId,
    forceRefreshSystemAuth,
    didFetchSystemAuth,
    entryReady,
...
const fetchBiometrics = async () => {
  if (isFetchingBiometricsRef.current) {
  ...
  try {
    await reconcileBiometricAuthState();
    recordAuthReadinessDiagnostic('biometrics-reconciled', {
      operationId,
      ...getBiometricsDiagnosticData(),
    });
    ...
    recordAuthReadinessDiagnostic('biometrics-system-auth-resolved', {
      operationId,
      didFetchSupportedType,
      devicePasscodeAvailable: systemAuth.devicePasscodeAvailable,
      supportedBiometryType: systemAuth.supportedBiometryType ?? null,
    });
    const enabledBySetting = isAuthenticatedByBiometrics();
    const entryReady = await checkAndroidBiometricsEntryReady();
    ...
    recordAuthReadinessDiagnostic('biometrics-fetch-finished', {
      operationId,
      entryReady,
      ...getBiometricsDiagnosticData(),
    });
  } catch (error) {
    recordAuthReadinessDiagnostic('biometrics-fetch-failed', {
      operationId,
      errorName: error instanceof Error ? error.name : 'unknown',
    });
    console.error(error);
...
const toggleBiometrics = async <T extends boolean>(
  ...
  const operationId = recordAuthReadinessDiagnostic(
    'biometrics-toggle-started',
    {
      ...
      requestedAuthenticationType:
        authenticationType === undefined ? null : String(authenticationType),
      ...getBiometricsDiagnosticData(),
    },
  );
  ...
  try {
    if (!nextEnabled) {
      await reset();
      recordAuthReadinessDiagnostic('biometrics-keychain-updated', {
        operationId,
        nextEnabled,
        ...getBiometricsDiagnosticData(),
      });
      ...
      recordAuthReadinessDiagnostic('biometrics-keychain-verified', {
        operationId,
        actionSuccess: !!(requestResult && requestResult.actionSuccess),
    ...
    recordAuthReadinessDiagnostic('biometrics-toggle-failed', {
      operationId,
      errorName: error instanceof Error ? error.name : 'unknown',
      nextEnabled,
    });
    ...
    recordAuthReadinessDiagnostic('biometrics-toggle-finished', {
      operationId,
      changed,
      nextEnabled,
      ...getBiometricsDiagnosticData(),
