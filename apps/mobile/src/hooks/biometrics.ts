...
export function computeBiometricsState({
  ...
  const isDevicePasscodeOnlyAvailable =
    !supportedBiometryType && devicePasscodeAvailable;
  const shouldUseDevicePasscodeLabel = isUsingDevicePasscode;
  ...
  const systemAuthTypeLabel = shouldUseDevicePasscodeLabel
    ? devicePasscodeLabel
    : detectedBiometricsLabel;
  const systemAuthSettingsLabel = shouldUseDevicePasscodeLabel
