...
async function persistWalletAuthPreferenceBeforeNavigation(
  formValues: PasswordFormValues,
  mode: WalletCreationMode,
  operationId = 0,
) {
  const passwordIsAutoGenerated = !!formValues.useAutoGeneratedPassword;
  await setPasswordIsAutoGeneratedDurably(passwordIsAutoGenerated);
  recordAuthReadinessDiagnostic('wallet-password-preference-persisted', {
    mode,
    operationId,
...
/**
 * Persists the "needs backup" reminder for a freshly created seed phrase.
 *
 * The reminder is keyed by basePublicKey, so every address of the seed phrase
 * shares it. Written here directly instead of relying on the `ACCOUNT_ADDED`
 * consumer, which is only registered once the account store lifecycle starts.
 */
async function markNewSeedPhraseNeedsBackup(address: string) {
  try {
    ...
    console.error('markNewSeedPhraseNeedsBackup error', error);
  }
}
...
async function submitCreateWallet(
  ...
  const result = await apisLock.resetPasswordOnUI(formValues.password, {
    passwordOrigin: formValues.useAutoGeneratedPassword
      ? 'auto-generated'
      : 'user',
  });
  if (result.error) {
    ...
    try {
      ...
      if (formValues.useAutoGeneratedPassword) {
        ...
        await keyringServiceApi.resetPassword(RABBY_MOBILE_KR_PWD, {
          passwordState: { version: 1, origin: 'built-in' },
        });
        throw e;
      }
  ...
  // 5. Mark the seed phrase as pending backup
  await markNewSeedPhraseNeedsBackup(address);
  ...
  await fetchLockInfo();
  await persistWalletAuthPreferenceBeforeNavigation(formValues, 'create');
...
async function submitImportSeedPhrase(
  ...
  await fetchLockInfo();
  await persistWalletAuthPreferenceBeforeNavigation(
    formValues,
    'importSeedPhrase',
  );
...
async function submitImportPrivateKey(
  ...
  const { toggleBiometrics } = callbacks;
  const operationId = recordAuthReadinessDiagnostic('wallet-setup-started', {
    enableBiometrics: !!formValues.enableBiometrics,
    mode: 'importPrivateKey',
    useAutoGeneratedPassword: !!formValues.useAutoGeneratedPassword,
  });
  ...
  // 1. Update password
  recordAuthReadinessDiagnostic('wallet-password-reset-started', {
    operationId,
    mode: 'importPrivateKey',
  });
  const result = await apisLock.resetPasswordOnUI(formValues.password, {
  ...
  recordAuthReadinessDiagnostic('wallet-password-reset-finished', {
    operationId,
    mode: 'importPrivateKey',
  });
  ...
  if (toggleBiometrics && formValues.enableBiometrics) {
    try {
      recordAuthReadinessDiagnostic('wallet-biometrics-toggle-started', {
        operationId,
        mode: 'importPrivateKey',
      });
      const ok = await toggleBiometrics(true, {
      ...
      recordAuthReadinessDiagnostic('wallet-biometrics-toggle-finished', {
        operationId,
        mode: 'importPrivateKey',
        succeeded: true,
      });
  ...
  recordAuthReadinessDiagnostic('wallet-keyring-imported', {
    operationId,
    mode: 'importPrivateKey',
  });
  ...
  // 8. Refresh lock info before navigating
  recordAuthReadinessDiagnostic('wallet-lock-refresh-started', {
    operationId,
    mode: 'importPrivateKey',
  });
  await fetchLockInfo();
  recordAuthReadinessDiagnostic('wallet-lock-refresh-finished', {
    operationId,
    mode: 'importPrivateKey',
  });
  await persistWalletAuthPreferenceBeforeNavigation(
  ...
  apisHomeTabIndex.setTabIndex(0);
  recordAuthReadinessDiagnostic('wallet-home-navigation-dispatched', {
    operationId,
    mode: 'importPrivateKey',
  });
}
