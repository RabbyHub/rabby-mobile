...
async function submitCreateWallet(
  ...
  // 1. Enable biometrics first — so rejection doesn't leave wallet with an unknown password.
  if (toggleBiometrics && formValues.enableBiometrics) {
    try {
      ...
      if (formValues.useAutoGeneratedPassword) throw e;
      // Manual mode: biometrics failure is non-fatal
