...
async function exportAndShareLocalStorageArchive() {
  ...
  try {
    ...
    // Storage enumeration and archive dependencies are user-demand work, not startup work.
    const { shareCurrentLocalStorageArchive } = await import(
      '@/core/storage/localStorageArchive'
    );
