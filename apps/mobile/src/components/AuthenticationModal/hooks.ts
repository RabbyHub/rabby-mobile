...
import { useAppLockState } from '@/hooks/appLockState';
...
import {
  coerceAuthType,
  filterAuthTypes,
  reconcileAuthTypeSelection,
} from './authType';

export { coerceAuthType, filterAuthTypes } from './authType';
...
export function useAuthTypeSelection(
  availableAuthTypes: apisLock.UIAuthType[],
) {
...
export function useAuthenticationModal(options: {
  ...
  const isUseCustomPwd = useAppLockState(
    state => state.pwdStatus === apisLock.PasswordStatus.Custom,
  );
  ...
  const { currentAuthType, setCurrentAuthType, updateAuthType } =
