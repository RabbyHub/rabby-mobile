...
} from '@/core/utils/authReadinessDiagnostics';
...
const DEFAULT_AUTH_TYPES: Exclude<apisLock.UIAuthType, 'none'>[] = [
  'biometrics',
  'password',
];
...
export interface AuthenticationModalProps extends ValidationBehaviorProps {
  ...
  authReadinessDiagnosticId?: number;
}
...
export const AuthenticationModal = ({
  ...
  $createParams,
  checklist,
  disableValidation: requestedDisableValidation,
  authReadinessDiagnosticId = 0,
  // disableValidation: propNoValidation = !validationHandler,
  authType: authTypes = DEFAULT_AUTH_TYPES,
...
>) => {
  ...
  const { isUseCustomPwd, pwdStatus } = usePasswordStatus();
  ...
  React.useEffect(() => {
    ...
    const anomalies = classifyAuthReadinessState({
      requestedAuthTypes: authTypes,
      availableAuthTypes,
      currentAuthType,
      pwdStatus: pwdStatusLabel,
    });
    recordAuthReadinessDiagnostic(
      hasRecordedMountRef.current
        ...
        : 'auth-modal-mounted',
      {
        ...
        biometricsEnabled: bioComputed.isBiometricsEnabled,
        currentAuthType,
        disableValidation,
        passwordIsAutoGenerated: getPasswordIsAutoGeneratedSnapshot(),
        pwdStatus: pwdStatusLabel,
        requestedAuthTypes: authTypes,
        requestedDisableValidation: requestedDisableValidation ?? null,
        variant: 'legacy',
      },
    );
  ...
  const handleConfirm = useCallback(() => {
    recordAuthReadinessDiagnostic('auth-modal-confirmed', {
      authReadinessDiagnosticId,
      availableAuthTypes,
      currentAuthType,
      disableValidation,
      pwdStatus: getPasswordStatusDiagnosticLabel(pwdStatus),
      requestedAuthTypes: authTypes,
      variant: 'legacy',
    });
...
AuthenticationModal.show = async (
  ...
  const requestedAuthTypes = showConfig.authType || ['biometrics', 'password'];
  const appLockState = getAppLockStateSnapshot();
  const biometricsState = getBiometricsInfoSnapshot();
  const authReadinessDiagnosticId = recordAuthReadinessDiagnostic(
    'auth-modal-show-requested',
    {
      authEnabled: biometricsState.authEnabled,
      devicePasscodeAvailable: biometricsState.devicePasscodeAvailable,
      passwordIsAutoGenerated: getPasswordIsAutoGeneratedSnapshot(),
      pwdStatus: getPasswordStatusDiagnosticLabel(appLockState.pwdStatus),
      requestedAuthTypes,
      requestedDisableValidation: showConfig.disableValidation ?? null,
      supportedBiometryType: biometricsState.supportedBiometryType ?? null,
      variant: 'legacy',
    },
  );
  let disableValidation = showConfig.disableValidation;
  const lockInfo = await apisLock.getRabbyLockInfo();
  recordAuthReadinessDiagnostic('auth-modal-native-state-resolved', {
    authReadinessDiagnosticId,
