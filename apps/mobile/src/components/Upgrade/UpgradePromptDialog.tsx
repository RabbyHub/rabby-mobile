...
export function UpgradePromptDialog({
  ...
  visible: boolean;
  remoteVersion: { version: string; changelog: string };
  onClose: () => void;
  onUpdate: () => void;
  modalId?: string;
}) {
  ...
  const parsedMarkdown = useMemo(
    () => parseMarkdown(remoteVersion.changelog),
    [remoteVersion.changelog],
  );

  useEffect(() => {
    if (visible && !parsedMarkdown.success) {
      onClose();
    }
  }, [visible, parsedMarkdown.success, onClose]);

  return (
    <TrackedModal
      modalId={modalId}
      visible={visible && parsedMarkdown.success}
      transparent
      animationType="fade"
