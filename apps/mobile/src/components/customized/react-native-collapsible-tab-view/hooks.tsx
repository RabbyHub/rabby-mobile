...
import { ONE_FRAME_MS } from 'react-native-collapsible-tab-view/src/helpers';
...
export const useScrollHandlerY = (
  name: TabName,
  {
    ...
    scrollableEnabled: propScrollableEnabled,
  }: ScrollHandlerProps = {},
) => {
  const {
    accDiffClamp,
  ...
  const enabled = propScrollableEnabled || internalEnabled;
  ...
  const onMomentumEnd = (event?: NativeScrollEvent, ctx?: any) => {
    'worklet';
    if (onAnimatedScrollMomentumEnd && event) {
      runOnJS(onAnimatedScrollMomentumEnd)(
        { nativeEvent: event } as Pick<
  ...
  const scrollHandler = useAnimatedScrollHandler(
    {
      onScroll: (event, ctx) => {
        ...
        if (focusedTab.value === name) {
          if (IS_IOS) {
            ...
            const clampMax = contentHeight - containerHeight + contentInset;
      ...
      onBeginDrag: (event, ctx) => {
        if (onAnimatedScrollBeginDrag) {
          runOnJS(onAnimatedScrollBeginDrag)(
            { nativeEvent: event } as Pick<
              NativeSyntheticEvent<NativeScrollEvent>,
              'nativeEvent'
            >,
      ...
      onEndDrag: (event, ctx) => {
        if (onAnimatedScrollEndDrag) {
          runOnJS(onAnimatedScrollEndDrag)(
            { nativeEvent: event } as Pick<
              NativeSyntheticEvent<NativeScrollEvent>,
              'nativeEvent'
            >,
      ...
      onMomentumBegin: (event, ctx) => {
        if (onAnimatedScrollMomentumBegin) {
          runOnJS(onAnimatedScrollMomentumBegin)(
            { nativeEvent: event } as Pick<
              NativeSyntheticEvent<NativeScrollEvent>,
              'nativeEvent'
            >,
  ...
  useAnimatedReaction(
    ...
    (isSyncNeeded, wasSyncNeeded) => {
      if (
        ...
        const focusedScrollY = scrollY.value[focusedTab.value] || 0;
        const tabScrollY = scrollY.value[name] || 0;
