...
build_cache_ensure_gradle_user_home() {
  if [ -L "$GRADLE_USER_HOME" ] && [ ! -e "$GRADLE_USER_HOME" ]; then
    gradle_user_home_target=$(readlink "$GRADLE_USER_HOME")
    case "$gradle_user_home_target" in
      /*) ;;
      *)
        gradle_user_home_target="$(dirname "$GRADLE_USER_HOME")/$gradle_user_home_target"
        ;;
    esac
    mkdir -p "$gradle_user_home_target"
    return
  fi
