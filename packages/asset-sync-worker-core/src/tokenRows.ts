...
export function makeTokenCacheRow(
  ...
  return {
    ...
    projection_resource_id: makeProjectionResourceId(
      ownerAddress,
      chain,
      id,
    ),
    ...
    inner_id: innerId,
    amount: numberValue(input.amount) * LEGACY_REAL_STORAGE_RATIO,
    chain,
    ...
    price: numberValue(input.price) * LEGACY_REAL_STORAGE_RATIO,
    symbol: input.symbol ?? '',
    time_at: input.time_at ?? 0,
    usd_value: input.usd_value ?? 0,
...
export function makeTokenCacheRows(
  ...
  const normalizedTokens = tokens.length ? tokens : [makeEmptyTokenInput()];
