...
import { MenuView } from '@rabby-wallet/react-native-menu';
...
const createAndroidMenu = (Menu: 'ContextMenu' | 'DropdownMenu') => {
  const Trigger = create(({ children, style, asChild }: MenuTriggerProps) => {
    if (asChild) {
      return cloneElement(children, {
        // @ts-expect-error - style type mismatch in React 18
        style,
      });
    }
  ...
  const Content = create(
    ({ children, getChildren }: MenuContentProps | ContextMenuContentProps) => {
...
<SubTrigger>
  ...
  const Root = create((props: MenuRootProps) => {
    ...
    const menuComponentRef =
      useRef<import('@rabby-wallet/react-native-menu').MenuComponentRef>(null);
  ...
  2. OR, use the textValue prop on <${Menu}.Item textValue="Some value" />`,
    ...
    const isDynamicMenu = typeof content?.props.getChildren === 'function';
    ...
    const getDynamicMenuItems = () => {
      clearCallbacks();
      return mapItemsChildren(content?.props.getChildren?.()).filter(
        filterNull,
      );
    };
    const menuItems = isDynamicMenu
      ? []
      : mapItemsChildren(content?.props.children).filter(filterNull);
    ...
    useImperativeHandle(androidMenuViewRef, () => ({
      show() {
        menuComponentRef.current?.show(
          isDynamicMenu ? getDynamicMenuItems() : undefined,
        );
      },
    }));

    const isAnchoredToRight = triggerItem?.props?.isAnchoredToRight;
    const androidSuppressNativeLongPress =
      triggerItem?.props?.androidSuppressNativeLongPress;
    return (
      <MenuView
        title={menuTitle}
