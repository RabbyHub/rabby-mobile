...
import {
  ...
  MenuConfig,
  MenuElementSize,
} from '@rabby-wallet/react-native-ios-context-menu'
import { create } from '../display-names'
import type { ImageSystemConfig } from '@rabby-wallet/react-native-ios-context-menu'

let dynamicMenuSequence = 0
...
<SubTrigger>
  ...
  // @ts-expect-error - defaultProps not available on FC in React 18
  ...
  const Root = create((props: MenuRootProps) => {
    ...
    const dynamicMenuIdRef = useRef<string | null>(null)
    if (dynamicMenuIdRef.current == null) {
      dynamicMenuSequence += 1
      dynamicMenuIdRef.current = `zeego-dynamic-menu-${dynamicMenuSequence}`
    }
...
3. OR, if you are using a horizontal Group with icons only, pass an empty string: <${Menu}.ItemTitle textValue="" />`
    ...
    const getDynamicMenuItems = () => {
      clearCallbacks()
      return mapItemsChildren(content?.props.getChildren?.()).filter(filterNull)
    }
    const menuItems: MenuConfig['menuItems'] = isDynamicMenu
      ? [
          {
            type: 'deferred',
            deferredID: dynamicMenuIdRef.current,
            shouldCache: false,
          },
        ]
      : (() => {
          clearCallbacks()
          return mapItemsChildren(content?.props.children).filter(filterNull)
        })()
    ...
    const unsafeOnRequestDeferredElement =
      props.__unsafeIosProps?.onRequestDeferredElement
    const onRequestDeferredElement = (
      deferredID: string,
      completion: Parameters<
        NonNullable<typeof unsafeOnRequestDeferredElement>
      >[1]
    ) => {
      if (
        isDynamicMenu &&
        ...
        completion(getDynamicMenuItems())
        return
      }

      unsafeOnRequestDeferredElement?.(deferredID, completion)
    }
    ...
    // @ts-expect-error - useRef requires initial value in strict mode
    ...
    return (
      <Component
        ...
        onRequestDeferredElement={onRequestDeferredElement}
      >
        {triggerItem}
      </Component>
    )
