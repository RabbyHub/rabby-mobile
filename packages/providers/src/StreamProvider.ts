...
export abstract class AbstractStreamProvider extends BaseProvider {
  ...
  constructor(
    ...
    pipeline(
      ...
      connectionStream,
      this._handleStreamDisconnect.bind(this, 'Rabby'),
    );
    ...
    pipeline(
      ...
      this._jsonRpcConnection.stream,
      this._handleStreamDisconnect.bind(this, 'Rabby RpcProvider'),
    );
   ...
   *
   * Calls `rabby_getProviderState` and passes the result to
  ...
  private _handleStreamDisconnect(streamName: string, error: Error | null) {
    let warningMsg = `Rabby: Lost connection to "${streamName}".`;
    if (error?.stack) {
      warningMsg += `\n${error.stack}`;
    }
