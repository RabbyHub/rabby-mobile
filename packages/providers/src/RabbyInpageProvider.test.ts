...
/**
 * For legacy purposes, RabbyInpageProvider retrieves state from the wallet
 * in its constructor. This operation is asynchronous, and initiated via
 * {@link RabbyInpageProvider._initializeStateAsync}. This helper function
 ...
 * `rabby_getProviderState` method. Further responses can be setup via the
 ...
 * initialization.  See {@link RabbyInpageProvider._initializeState}.
...
async function getInitializedProvider({
  ...
  initialState?: Partial<
    Parameters<RabbyInpageProvider['_initializeState']>[0]
  >;
  onMethodCalled?: {
    substream: string;
    method: string;
  ...
  const connectionStream = new MockConnectionStream((name, data) => {
    if (
      ...
      setTimeout(() =>
        connectionStream.reply('rabby-provider', {
...
describe('RabbyInpageProvider: RPC', () => {
  const MOCK_ERROR_MESSAGE = 'Did you specify a mock return value?';

  /**
   * Creates a new RabbyInpageProvider instance, with a mocked connection
   * stream.
   *
   * @returns The new RabbyInpageProvider instance.
   */
  function initializeProvider() {
    const mockStream = new MockConnectionStream();
    const provider: any | RabbyInpageProvider = new RabbyInpageProvider(
      mockStream,
    );
  ...
  describe('integration', () => {
    let provider: any | RabbyInpageProvider;
...
describe('RabbyInpageProvider: Miscellanea', () => {
  describe('constructor', () => {
    it('succeeds if stream is provided', () => {
      expect(
        () => new RabbyInpageProvider(new MockConnectionStream()),
      ).not.toThrow();
    });
    ...
    it('succeeds if stream and valid options are provided', () => {
      ...
      expect(
        () =>
          new RabbyInpageProvider(stream, {
    ...
    it('throws if no or invalid stream is provided', () => {
      expect(() => new RabbyInpageProvider(undefined as any)).toThrow(
        messages.errors.invalidDuplexStream(),
      );

      expect(() => new RabbyInpageProvider('foo' as any)).toThrow(
        messages.errors.invalidDuplexStream(),
      );

      expect(() => new RabbyInpageProvider({} as any)).toThrow(
        messages.errors.invalidDuplexStream(),
      );
    });
    ...
    it('gets initial state', async () => {
      ...
      const requestMock = jest
        .spyOn(RabbyInpageProvider.prototype, 'request')
