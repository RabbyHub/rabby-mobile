...
class ResultHandlerInteractiveBiometricManualRetry(
  ...
  deviceCredentialFallbackPromptInfo: BiometricPrompt.PromptInfo? = null
) :
  ResultHandlerInteractiveBiometric(
    reactContext,
    storage,
    promptInfo,
  ...
  /** Trigger interactive authentication from an existing prompt callback. */
  protected override fun startAuthenticationWithoutBlocking() {
    val activity = getCurrentActivity()
