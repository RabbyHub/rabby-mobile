...
import DisplayKeyring, { Account } from './display';
import {
  isSameAddress,
  normalizeAddress,
  KEYRING_TYPE,
...
type OnGenerateAliasNameFunction = (params: {
  keyringType: string;
  brandName?: string | undefined;
  keyringCount?: number | undefined;
  addressCount?: number | undefined;
  address: string;
}) => string;

type OnSetAddressAliasFunction = (
  address: string,
  keyring: DisplayKeyring,
  brandName: string,
) => void;
...
export class KeyringController extends EventEmitter {
  ...
  importPrivateKey(privateKey: string): Promise<any> {
    ...
    return this.persistAllKeyrings()
      ...
      .then(async _keyring => {
        ...
        this.onGenerateAliasName({
          keyringType: KEYRING_TYPE.SimpleKeyring,
          keyringCount:
            keyrings.filter(k => k.type === KEYRING_TYPE.SimpleKeyring).length -
            1,
          addressCount: 0,
          address,
        });
  ...
  async getPreMnemonics(): Promise<any> {
    ...
    if (!this.password) {
      throw new Error(ERROR_MESSAGE.LOCKED);
    }
  ...
  addKeyring(keyring: DisplayKeyring) {
    return keyring
      .getAccounts()
      .then(accounts => {
  ...
  async verifyPassword(password: string): Promise<void> {
    ...
    if (!encryptedBooted) {
      throw new Error(ERROR_MESSAGE.CANNOT_LOCK);
    }
  ...
  async checkForDuplicate(
    ...
    const accounts: string[] = _accounts
      ...
      .map((address: string) => normalizeAddress(address).toLowerCase());
    ...
    return isIncluded
      ? Promise.reject(new Error(ERROR_MESSAGE.DUPLICATE))
   ...
   */
  signMessage(msgParams: { from: string | number; data: any }, opts = {}) {
   ...
   */
  decryptMessage(msgParams: { from: string | number; data: any }, opts = {}) {
    const address = normalizeAddress(msgParams.from);
    return this.getKeyringForAccount(address).then(keyring => {
  ...
  getKeyringForAccount(
    ...
    return Promise.all(
      ...
      const winners = candidates.filter(candidate => {
        const accounts = candidate[1].map((addr: string) => {
